...
export default function Stage30Page() {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      ...
      // 1️⃣ 이전 단계(50개)에서 선택한 song_id 가져오기
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setMessage("30개 선택 저장 완료!");
      window.location.href = `/stage10?session_id=${sessionId}`;
    }
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "16px", fontWeight: "bold" }}>
        선택 개수: {selected.length} / 30
      </div>

      <button
        onClick={handleSave}
        disabled={saving || selected.length !== 30}
        style={{
          marginBottom: "20px",
          padding: "10px 16px",
backgroundColor:
