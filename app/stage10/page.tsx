...
export default function Stage10Client() {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setMessage("10개 선택 저장 완료!");
      window.location.href = `/stage5?session_id=${sessionId}`;
    }
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "16px", fontWeight: "bold" }}>
        선택 개수: {selected.length} / 10
      </div>

      <button
        onClick={handleSave}
        disabled={saving || selected.length !== 10}
        style={{
          marginBottom: "20px",
          padding: "10px 16px",
          border: "none",
          borderRadius: "8px",
          fontWeight: "bold",
          cursor: saving || selected.length !== 10 ? "not-allowed" : "pointer",
          backgroundColor:
            selected.length === 10 && !saving ? "#444" : "#ccc",
          color:
            selected.length === 10 && !saving ? "#fff" : "#666",
        }}
      >
        {saving ? "저장 중..." : "10곡 저장하기"}
      </button>
